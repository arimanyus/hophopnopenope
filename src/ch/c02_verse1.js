...
(() => {
  ...
  function calmLeft(ctx, k = 1, x1 = 1000, color = INK.ink) {
    if (k <= .01) return;
    ctx.save(); ctx.setTransform(1, 0, 0, 1, 0, 0);
    dotsIn(ctx, [0, 0, x1, H], { spacing: 30, color, k: x => k * clamp(1.25 * (1 - x / x1)) });
    ctx.restore();
  }
  ...
  function ears(ctx, t) {
    ...
    const tall = seg(t, at(T), at(T) + .1), out = t - (at(T) + .2), twitch = out > 0 ? 12 * Math.exp(-out * 5) * Math.sin(out * 30) + wob(tc, 1.7) * 4 : 0;
    const pose = {
      ...
      earL: { a: lerp(-4, -40, flop), b: lerp(-wobE - twitch * .5, -48, flop), len }, earR: { a: lerp(4, 38, flop), b: lerp(wobE + twitch * .5, 50, flop), len },
      armL: { a: 12, e: 30 }, armR: { a: 12, e: 30 }, col: { fur: mix(INK.fur, INK.cyan, .1) }, noShadow: true,
    };
    const A = rabbit(ctx, 1400, 1260, 80, pose);
    ...
    if (out > 0 && out < .5) krackle(ctx, 1400, SURF, 140 + out * 260, { n: 20, size: 16 * (1 - out * 1.6), color: EARTH2 });
    ...
    if (out > 0) earSense(ctx, [[A.earL[0], A.earL[1] + 60], [A.earR[0], A.earR[1] + 60]], clamp(out / .1), 55);
  ...
  function smallFix(ctx, t) {
    ...
    rabbit(L, 1250 - lean * 60, 1640 + lean * 60, 118 * (1 + .22 * lean), { flip: true, tilt: -12 * lean, lids: .15 + .1 * lean, brows: .3 * lean, lx: .3, ly: .1,
      mouth: 'smile', eyes: blink(tc, [13.95]) > .5 ? 'closed' : 'open', noShadow: true, armL: { a: 10, e: 10 }, armR: { a: 10, e: 10 } });
    L.globalCompositeOperation = 'source-atop'; L.globalAlpha = .55; L.fillStyle = INK.nightLt; L.fillRect(0, 0, W, H);
    popLayer();
    ctx.save(); ctx.setTransform(1, 0, 0, 1, 0, 0); ctx.globalAlpha = .3; ctx.globalCompositeOperation = 'multiply'; ctx.drawImage(L.canvas, 0, 0); ctx.restore();
  ...
  const DIG = '14203', DIG_T = [14.44, 14.52, 14.62, 14.76, 14.9];
  ...
  const TAPE = [[1410, 560], [1395, 680], [1340, 820], [1280, 960], [1180, 1030], [900, 1045], [400, 1050], [-100, 1052], [-600, 1054]];
  const TAPE_TXT = ['+ console.log("here");', '\uD83E\uDD16 Co-authored-by: 64 agents', '+ return a ? b ? c : d : e;', '+ // TODO: remove before merge', '\uD83E\uDD16 Co-authored-by: 64 agents', '+ await sleep(1000); // fixes race'];
  function tape(ctx, t, t0) {
    ...
    for (let i = 0; i < pts.length - 1; i += 3) { const s = acc[i] + scroll, [x, y] = pts[i], [x2, y2] = pts[Math.min(pts.length - 1, i + 1)], ang = Math.atan2(y2 - y, x2 - x);
      if (y > 1000) continue; const kind = hash(Math.floor(s / 18)); ctx.save(); ctx.translate(x, y); ctx.rotate(ang);
      fillPts(ctx, rect(-9, -44, 12, 88 * (.4 + .6 * kind)), kind < .6 ? '#8FD9AE' : '#F4A6B0', false); ctx.restore(); }
    ctx.save(); clipPts(ctx, rect(-800, 990, 2000, 110), false);
    const tx = -mod(scroll, 2400);
    for (let r = 0; r < 4; r++) TAPE_TXT.forEach((s, i) => txt(ctx, s, 1200 + tx - (r * TAPE_TXT.length + i) * 800 + 2400 * 2, 1062, { font: 'mono', weight: 800, size: 44, color: s[0] === '+' ? '#0E6B3A' : INK.ink }));
    ctx.restore();
    outline(ctx, [...pts.map(([x, y]) => [x, y - 59]), ...pts.slice().reverse().map(([x, y]) => [x, y + 59])], 4, INK.ink, { smooth: false, heavy: .2 });
  }
  function fourteen(ctx, t) {
    const LN = wt(3, 4), tc = tw(t, at(LN)), pull = expoOut(seg(t, at(LN), at(LN) + .5));
    const [qx, qy] = shake(t, 14 * hit(t, [LN], 9) + 3 * hit(t, DIG_T, 14));
    const z = lerp(2.9, 1.18, pull), cx = lerp(1330, 1310, pull), cy = lerp(430, 600, pull);
    ...
    burrow(ctx, t, { pinsFocus: true, fg: false, screen: onScreen((c, vw, vh) => {
      ...
      odometer(c, 330, 380, 330, t);
      txt(c, '\u221212', 1520, 1010, { font: 'mono', weight: 800, size: 90, color: INK.red, align: 'right' });
      const over = seg(t, at(LN) - .2, at(LN) + .3); for (let i = 0; i < 5 + over * 30; i++) fillPts(c, rect(70 + i * 62, 900, 50, 50), INK.green, false);
    }) });
  ...
  function ternary(ctx, t) {
    ...
    for (let n = n0; n < n0 + 8; n++) {
      ...
      ink(ctx, rect(X, Y, PW, PH), { fill: mod(n, 2) ? '#221C33' : '#2E2545', line: 8 / sc > 30 ? 30 * sc / sc : 8, lineColor: INK.paper, boil: .6, smooth: false });
      const f = { font: 'mono', weight: 800, size: PH * .15 };
      txt(ctx, letters[mod(n, 26)] + ' ', X + 40, Y + PH * .2, { ...f, color: INK.paper });
      txt(ctx, '?', X + 40 + measure(ctx, 'a ', f).w, Y + PH * .2, { ...f, color: INK.orange });
      txt(ctx, ':', X + 40, Y + PH * .93, { ...f, color: INK.cyan });
      txt(ctx, ' ' + letters[mod(n + 13, 26)], X + 40 + measure(ctx, ':', f).w, Y + PH * .93, { ...f, color: INK.paper });
      ctx.restore();
    }
  ...
  const CELL = 80, COLS = 7, WX = 1000, WB = 980;
  const LOGS = (() => {
    const hero = [['"here"', 3, 0, 0], ['"here2"', 3, 0, 1], ['"HERE???"', 3, 3, 0], ['"why"', 2, 3, 1], ['"asdf"', 3, 0, 2], ['"it works??"', 4, 3, 2]];
    const out = hero.map(([s, w, c, r], i) => ({ s, w, c, r, T: 19.72 + i * .17, hero: 1 }));
    const tiny = ['"x"', '"1"', '"?"', '"a"', '"ok"', '"no"', '"hi"', '"2"', '".."', '"3"', '"!!"', '"k"'];
    const fill = [[6, 0, 1], [5, 1, 2]];
    let i = 0;
    for (const [c, r, w] of fill) out.push({ s: tiny[i++ % tiny.length], w, c, r });
    for (let r = 3; out.length < 45; r++) { let c = 0; while (c < COLS && out.length < 45) { const w = Math.min(COLS - c, 1 + Math.floor(hash(r * 13 + c * 7) * 2.2)); out.push({ s: tiny[Math.floor(hash(r * 3 + c) * tiny.length)], w, c, r }); c += w; } }
    out.slice(6).forEach((p, j, a) => { p.T = 20.66 + j / a.length * .8; });
    const top = Math.max(...out.map(p => p.r)) + 1;
    out.push({ s: '"\uD83D\uDE4F"', w: 2, c: 3, r: top, T: 21.52, hero: 1 });
    return out;
  })();
  function consoleLogs(ctx, t) {
    ...
    // the well, on a CRT
    const top = WB - 11 * CELL;
    ink(ctx, rrect(WX - 40, top - 70, COLS * CELL + 80, 11 * CELL + 130, 20), { fill: '#D9CDB4', shade: { color: rgba(INK.ink, .35), spacing: 14, dir: [.6, .8], from: -40, to: 800 }, line: 6, boil: 1, smooth: false });
    fillPts(ctx, rect(WX, top - 30, COLS * CELL, 11 * CELL + 30), '#101018', false);
    ctx.save(); clipPts(ctx, rect(WX, top - 30, COLS * CELL, 11 * CELL + 30), false);
    for (let c = 1; c < COLS; c++) fillPts(ctx, rect(WX + c * CELL - 1, top - 30, 2, 11 * CELL + 30), '#1E1E2C', false);
    const cols = [INK.orange, INK.yellow, INK.cyan, INK.paper];
    LOGS.forEach((p, i) => {
      const g = t - at(p.T), fd = p.hero ? .32 : .14; if (g < -fd) return;
      const ty = WB - (p.r + 1) * CELL, y = g >= 0 ? ty : top - CELL + Math.floor(((g + fd) / fd) * ((ty - top + CELL) / CELL)) * CELL;
      const x = WX + p.c * CELL, w = p.w * CELL, fillc = cols[Math.floor(hash(i * 3.3) * cols.length)];
      ...
      txt(ctx, p.s, x + w / 2, y + CELL * .66, { font: 'mono', weight: 800, size: p.hero ? 38 : 34, align: 'center', color: INK.ink });
    });
    ctx.restore();
    ctx.save(); ctx.globalAlpha = .1; for (let y = top - 30; y < WB; y += 6) fillPts(ctx, rect(WX, y, COLS * CELL, 2), INK.white, false); ctx.restore();
    // counter
    ink(ctx, rrect(1600, 110, 240, 230, 16), { fill: INK.ink, line: 5, boil: .6, smooth: false });
    txt(ctx, 'console.log', 1720, 160, { font: 'mono', weight: 800, size: 30, color: INK.paper, align: 'center' });
    txt(ctx, String(n), 1720, 300, { font: 'mono', weight: 800, size: 130 * (1 + .12 * hit(t, LOGS.map(p => p.T), 18)), color: n >= 46 ? INK.orange : INK.yellow, align: 'center' });
    // the rabbit counts on its paws, then gives up
    const f = tc > 20.6 && tc < at(21.52), done = tc >= at(21.52), cnt = n % 3;
    rabbit(ctx, 1720, 1000, 30, { turn: -.5, lx: -.9, ly: done ? -.9 : -.4 + .5 * wob(tc, 2), eyes: f ? 'wide' : 'open', lids: done ? .5 : 0, mouth: done ? 'flat' : f ? 'wavy' : 'o', open: .3, sweat: f ? .8 : 0,
  ...
  function dj(ctx, t) {
    ...
    ctx.restore();
    // the DJ
    const nod = .5 * pulse(tc, 6), fad = .5 + .5 * wob(t, 1.2);
    const A = rabbit(ctx, 1420, 1040, 38, { nod, tilt: wob(tc, 1.2) * 6, eyes: 'happy', mouth: 'grin', blush: .5,
      armL: { a: 150, e: 95 }, pawL: 'mitt', armR: { a: 35, e: 45 + fad * 25 }, pawR: 'fist', earL: { a: -14, b: -18 * p }, earR: { a: 12, b: 18 * p } });
    headphones(ctx, A, 38, wob(tc, 1.2) * 6);
    // the mixing console
    const top = 800, poly = [[1000, top], [1860, top], [1930, 1100], [920, 1100]];
    ink(ctx, poly, { fill: '#2A2638', shade: { color: INK.ink, spacing: 14, dir: [0, 1], from: 0, to: 300 }, line: 6, boil: .8, smooth: false });
    for (let i = 0; i < 8; i++) {
      const x = 1045 + i * 100, lv = clamp(p * (.55 + .45 * hash(i + beatN(t) * 7)) + .15 * noise1(t * 9 + i));
      for (let j = 0; j < 8; j++) fillPts(ctx, rect(x, top + 150 - j * 16, 26, 11), j / 8 < lv ? (j > 6 ? INK.red : j > 4 ? INK.yellow : INK.green) : '#3E3A50', false);
      fillPts(ctx, rect(x + 40, top + 30, 8, 150), INK.ink, false);
      const fy = top + 40 + (i === 5 ? fad : hash(i * 2.7)) * 110; ink(ctx, rect(x + 28, fy, 32, 22), { fill: INK.paper, line: 3, boil: .4, smooth: false });
      txt(ctx, 'console.log', x + 16, top + 290, { font: 'mono', weight: 800, size: 22, color: INK.paper, rot: -PI / 2 });
    }
    ink(ctx, rrect(1150, 1010, 560, 64, 10), { fill: INK.paper, line: 4, boil: .5, smooth: false });
    txt(ctx, 'console.log \u00D7 46', 1430, 1058, { font: 'mono', weight: 800, size: 44, color: INK.ink, align: 'center' });
    calmLeft(ctx, .75, 1000);
  }

  // ================= 9 · A SECRET KEY COMMITTED (23.40-25.55) =================
  const KEYEND = [1470, 460, 330, -.12];
  ...
  function secretKey(ctx, t) {
    ...
    depth(ctx, 8, c => { cam(c, lerp(960, 1040, push) + qx, 540 + qy, lerp(1, 1.06, push));
      const lines = fakeDiff(24, 9); for (let i = -8; i < 12; i++) { if (!i) continue; const y = ky + i * LH - scroll; const [k2, s2] = lines[mod(i + 8, 24)]; fillPts(c, rect(-100, y, 2200, LH), k2 === '+' ? '#D8F3E1' : k2 === '@' ? '#E7E2F7' : '#F4F1EA', false);
        txt(c, String(311 + i), 150, y + LH * .66, { font: 'mono', weight: 600, size: 40, color: '#8A8496', align: 'right' }); txt(c, (k2 === '@' ? '' : k2 + ' ') + s2, 190, y + LH * .66, { font: 'mono', weight: 700, size: 46, color: '#0E6B3A' }); }
      c.restore(); });
    ...
    if (kk > 0) {
      const [kx, ky2, ks, kr] = KEYEND, x = lerp(1560, kx, kk), yy = lerp(y + 46, ky2, kk);
      goldKey(ctx, x, yy, ks * kk, kr * kk + wob(tc, .8) * .04);
      for (let i = 0; i < 3; i++) sparkle(ctx, x + [-120, 90, 150][i], yy + [-70, -60, 50][i], 36, clamp(Math.sin((tc * 3 + i * .33) * PI)) * kk, i);
    }
    ...
    stamp(ctx, 'CRITICAL', 1090, 330, 150, t - at(CM), { color: INK.red, rot: -.1 });
    if (hit(t, [CM], 20) > .3) misregFrame(ctx, 12 * hit(t, [CM], 20));
  }

  // ================= 10 · PLAIN AS DAY (25.55-27.30) =================
  function plainAsDay(ctx, t) {
    const tc = twos(t), pull = easeInOut(seg(t, 25.55, 26.5));
    const BK = [1250, 330, 520, -.08];                                  // key on the billboard (world)
    ...
    uiBox(ctx, 740, 60, 1120, 580, { fill: INK.white, shadow: 18, line: 8, r: 6 });
    goldKey(ctx, BK[0], BK[1], BK[2], BK[3] + wob(tc, .5) * .02);
    txt(ctx, 'sk_live_HOPHOPNOPE_4f9\u2026', 1300, 580, { font: 'mono', weight: 800, size: 64, color: INK.ink, align: 'center' });
    for (let i = 0; i < 4; i++) sparkle(ctx, 1000 + i * 180, 170 + (i % 2) * 280, 40, clamp(Math.sin((tc * 2.5 + i * .25) * PI)), i);
    ctx.restore();
    const [qx] = shake(t, 0);
  }
  ...
  function ciPanel(ctx, t, o = {}) {
    const x = 980, y = 130, w = 840;
    uiBox(ctx, x, y, w, 820, { fill: INK.white, shadow: 16, line: 6, r: 14 });
    ink(ctx, ell(x + 80, y + 90, 44, 44, 24), { fill: INK.green, line: 5, boil: .5 });
    txt(ctx, '\u2713', x + 80, y + 112, { font: 'ui', weight: 900, size: 60, color: INK.white, align: 'center' });
    txt(ctx, 'All checks have passed', x + 150, y + 112, { font: 'ui', weight: 800, size: 60, color: INK.ink });
    txt(ctx, '412 successful checks', x + 150, y + 170, { font: 'ui', weight: 600, size: 34, color: '#6E6A78' });
  ...
  function testsPass(ctx, t) {
    ...
    ctx.restore();
    const relax = n / 4;
    rabbit(ctx, 1760, 1040, 30, { turn: -.4, lean: -8 * relax, eyes: relax > .6 ? 'happy' : 'open', lids: .2 * relax, mouth: 'smile', blush: .5 * relax,
  ...
  const SKIPS = [["describe.skip('payments', () => {", INK.ink], ["  it.skip('charges the card')", INK.ink], ["  xit('handles null')", INK.ink], ["  it.skip('refunds')", INK.ink], ['});', INK.ink]];
  ...
  function skipped(ctx, t) {
    ...
    SKIPS.forEach(([s], i) => { const f = { font: 'mono', weight: 800, size: 40 }; txt(ctx, s, 1010, 240 + i * 70, { ...f, color: INK.paper });
      const m = s.match(/\.skip|xit/); if (m) { const pre = measure(ctx, s.slice(0, m.index), f).w; txt(ctx, m[0], 1010 + pre, 240 + i * 70, { ...f, color: INK.yellow }); } });
    const res = backOut(seg(t, at(AN), at(AN) + .16), 2.2);
    if (res > 0) { ctx.save(); ctx.translate(1410, 650); ctx.scale(res, res); txt(ctx, '0 passed \u00B7 412 skipped', 0, 0, { font: 'mono', weight: 800, size: 60, color: INK.yellow, align: 'center', stroke: { w: 8, color: INK.ink } }); ctx.restore(); }
    // floor
    ...
    for (let i = 4; i >= 0; i--) { const t0 = at(SK) + i * .09, g = t - t0, fall = g < 0 ? 0 : Math.min(1, easeIn(g / .2)) * (PI / 2 - .12 * (i < 4 ? 1 : 0)) - (g > .2 ? .08 * Math.exp(-(g - .2) * 12) * Math.sin((g - .2) * 40) : 0);
      checkStandee(ctx, 1150 + i * 150, 885, 230, fall); }
    ctx.restore();
  ...
  const COUNT = [31.44, 31.65, 31.86];
  function pen(ctx, t) {
    const tc = twos(t), n = COUNT.filter(T => t >= at(T)).length;
    const punch = expoIn(seg(t, at(COUNT[2]) + .02, 32.09));
    const hx = 960, hy = 150, size = 66, f = { font: 'ui', weight: 800, size };
    const lw = measure(ctx, 'Actionable comments posted: ', f).w, tot = lw + measure(ctx, '3', { ...f, weight: 900, size: size * 1.25 }).w, x0 = hx - tot / 2, nx = x0 + lw + size * .4, ny = hy - size * .35;
    cam(ctx, lerp(960, nx, punch), lerp(540, ny, punch), lerp(1, 14, punch));
    depth(ctx, 8, c => { cam(c, lerp(960, nx, punch), lerp(540, ny, punch), lerp(1, 14, punch)); cam(c, 1200, 520, 1.5); burrow(c, t, { pinsFocus: true, fg: false }); c.restore(); c.restore(); });
    // the desk and the draft
    ink(ctx, [[300, 830], [1620, 830], [1760, 1100], [160, 1100]], { fill: '#8A5A34', shade: { color: '#4A2C18', spacing: 14, dir: [0, 1], from: 0, to: 300 }, line: 6, boil: 1, smooth: false });
    ink(ctx, [[620, 850], [1300, 850], [1340, 1060], [580, 1060]], { fill: INK.white, line: 4, boil: .6, smooth: false });
    const wr = seg(t, at(30.82), at(COUNT[2]));
    for (let i = 0; i < 5; i++) { const lk = clamp(wr * 5 - i); if (lk <= 0) continue; const y = 895 + i * 34, x1 = lerp(640, 1280, lk);
      inkLine(ctx, [[640, y], ...Array.from({ length: 10 }, (_, j) => [lerp(640, x1, (j + 1) / 10), y + Math.sin(j * 2.3 + i) * 6])], 5, INK.red, { taper: [0, .2] }); }
    // the rabbit
    const reach = seg(tc, 30.1, 30.34), pluck = t >= at(30.4), twirl = seg(t, 30.42, 30.6), crack = seg(tc, 30.6, 30.82), writing = t >= at(30.82) && punch < .2;
    let pose = { eyes: 'open', mouth: 'smirk', brows: -.1, browTilt: 1.2, lids: .2, earL: { a: -10, b: 0 }, earR: { a: 10, b: 0 }, pen: !pluck, noShadow: true };
    if (!pluck) pose = { ...pose, tilt: 10 * reach, armR: { a: lerp(10, 168, easeOut(reach)), e: lerp(20, -25, reach) }, pawR: 'mitt', lx: .6 * reach, ly: -.6 * reach };
    else if (crack > 0 && crack < 1) pose = { ...pose, armL: { a: 55, e: 110 - 20 * wob(tc, 4) }, armR: { a: 55, e: 110 - 20 * wob(tc, 4) }, pawL: 'fist', pawR: 'fist', eyes: 'closed', mouth: 'grin' };
    else if (writing) { const w1 = wob(t, 7.5), w2 = wob(t, 11, .3); pose = { ...pose, lx: .3 * w1, ly: .9, nod: .25, armR: { a: 28 + 12 * w1, e: 55 + 25 * w2 }, pawR: 'fist', armL: { a: 30, e: 70 }, pawL: 'mitt', mouth: 'open', open: .2, sweat: .5 }; }
    else pose = { ...pose, armR: { a: 110, e: 60 }, pawR: 'fist', armL: { a: 20, e: 30 } };
    const A = rabbit(ctx, 960, 1000, 62, pose);
    if (pluck && !(crack > 0 && crack < 1)) {
      const ang = writing ? deg(125) + wob(t, 7.5) * .3 : -PI / 2 - twirl * TAU;
      if (writing) for (let g = 3; g >= 1; g--) { ctx.save(); ctx.globalAlpha = .18 * g; const tt = t - g / 48, w1 = wob(tt, 7.5), w2 = wob(tt, 11, .3); ctx.restore(); }
      penAt(ctx, A.pawR[0], A.pawR[1], 62, ang);
    }
    if (crack > .35 && crack < .9) { sfx(ctx, 'KRAK', 960, 690, 90, (tc - 30.7), { rot: -.08, life: .3 }); krackle(ctx, 960, 740, 90, { n: 16, size: 10 }); }
    if (writing) speedLines(ctx, A.pawR[0], A.pawR[1], { n: 24, r0: 70, r1: 220, w: 5, color: INK.ink });
    // the counter: 0 -> 1 -> 2 -> 3 on the last snare hits
    const bump = hit(t, COUNT, 12), show = seg(t, 30.78, 30.9);
    if (show > 0) { ctx.save(); ctx.globalAlpha = show;
      ink(ctx, rrect(x0 - 50, hy - size * 1.25, tot + 100, size * 1.8, 18), { fill: INK.white, line: 5, boil: .6, smooth: false });
      countHeader(ctx, x0, hy, n, { size, numScale: 1.25 * (1 + .35 * bump) }); ctx.restore(); }
    ctx.restore();
    if (punch > .5) misregFrame(ctx, 14 * punch);
  }
