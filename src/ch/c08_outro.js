...
(() => {
  ...
  function revertPR(ctx, t) {
    const X = 170;
    uiBox(ctx, 100, 90, 1760, 1200, { r: 16, shadow: 18 });
    coolShade(ctx, rrect(100, 90, 1760, 1200, 16));
    ...
    ctx.save(); ctx.translate(X, 560); ctx.scale(ds, ds);
    ...
    ctx.restore();
    prTabs(ctx, X, 520, 1620, { tabs: [['Conversation', 2], ['Commits', 1], ['Checks', 14], ['Files changed', 347]], active: 0, size: 30 });
    // merge box: checks go green on stab 2
    const a2 = age(t, STAB[1]), ok = a2 >= 0;
    uiBox(ctx, X, 660, 1620, 430, { r: 12, shadow: 0 });
    checkDisc(ctx, 262, 750, 54 * slam(a2, [1.5, .9, 1.06]), ok);
    if (ok && a2 < .3) ring(ctx, 262, 750, 60 + a2 * 1100, 12 * (1 - a2 / .3), INK.green);
    txt(ctx, ok ? 'All checks have passed' : 'Some checks haven\u2019t completed yet', 350, 768, { font: 'ui', weight: 800, size: 68 });
    txt(ctx, ok ? '14 successful checks' : '3 in progress', 350, 822, { font: 'ui', weight: 500, size: 34, color: GREY });
    inkLine(ctx, [[X, 860], [X + 1620, 860]], 3, '#D8D2C4', { taper: [0, 0], smooth: false });
    checkDisc(ctx, 262, 910, 30, true);
    txt(ctx, 'This branch has no conflicts with the base branch', 350, 922, { font: 'ui', weight: 600, size: 32 });
    inkLine(ctx, [[X, 958], [X + 1620, 958]], 3, '#D8D2C4', { taper: [0, 0], smooth: false });
    button(ctx, 350, 985, 470, 84, 'Merge pull request', { fill: INK.green, size: 34 });
  }
  ...
  const MCAM = [[930, 340, 1.22, -.012], [800, 850, 1.5, .01], [440, 300, 2.6, -.018], [2490, 440, 2.0, .012]];
  ...
  const CX = 200, CY = 280, CW = 900, RY = CY + 250, RBX = CX + 84;
  ...
  function addReaction(ctx, x, y, hov) {
    ink(ctx, rrect(x, y - 24, 64, 48, 24), { fill: hov ? '#E4DED0' : '#EFEAE0', line: 3, smooth: false, boil: .4 });
    outline(ctx, ell(x + 26, y, 12, 12, 16), 3, INK.ink);
    fillPts(ctx, ell(x + 22, y - 3, 2, 2.5, 6), INK.ink); fillPts(ctx, ell(x + 30, y - 3, 2, 2.5, 6), INK.ink);
    inkLine(ctx, [[x + 20, y + 4], [x + 26, y + 8], [x + 32, y + 4]], 2.5, INK.ink, { taper: [0, 0] });
    inkLine(ctx, [[x + 46, y - 10], [x + 54, y - 10]], 3, INK.ink, { taper: [0, 0], smooth: false }); inkLine(ctx, [[x + 50, y - 14], [x + 50, y - 6]], 3, INK.ink, { taper: [0, 0], smooth: false });
  }
  function eyesChip(ctx, x, y, a) {
    const f = { font: 'ui', weight: 700, size: 32 }, s = '\uD83D\uDC40 1', w = measure(ctx, s, f).w + 36, k = slam(a, [1.4, .9, 1.05]);
    ctx.save(); ctx.translate(x + w / 2, y); ctx.scale(k, k);
    ink(ctx, rrect(-w / 2, -24, w, 48, 24), { fill: '#FFE3EE', line: 3, lineColor: INK.pink, smooth: false, boil: .4 });
    txt(ctx, s, 0, 11, { ...f, align: 'center' });
    ctx.restore();
  }
  function thread(ctx, t, hov) {
    ...
    pawPrint(ctx, CX + CW - 110, CY + 150, 1, .35);
    addReaction(ctx, RBX, RY, hov);
    const ac = age(t, T_CLICK); if (ac >= 0) eyesChip(ctx, RBX + 80, RY, ac);
    commentCard(ctx, CX, CY + 310, CW, t, { author: 'coderabbitai', time: 'just now', body: ['As mentioned above.'], buttons: false, size: 34 });
  }
  function react(ctx, t) {
    const a = Math.max(0, age(t, T_CMT)), p = Math.exp(-a * 12), k = easeInOut(clamp(a / 1.55));
    const zoom = lerp(1.9, 2.2, k) * (1 + .07 * p + .025 * kick(t, [T_CLICK], 8) + .012 * kick(t, [F(2979), F(2984)], 10));
    const cx = lerp(660, 560, k), cy = lerp(470, 480, k);
    nightField(ctx, cx, cy, zoom);
    cam(ctx, cx, cy, zoom, lerp(.012, -.004, k));
    const P = kf(t, [[T_CMT, [820, 830]], [124.42, [420, 610]], [124.64, [RBX + 34, RY + 4]], [125.05, [RBX + 34, RY + 4]], [125.45, [560, 820]]]);
  ...
  // ---------- the ending fairy ----------
  function fairyBg(ctx, t) {
    cam(ctx, 660, 330, 1.75);
    dep(ctx, 9, c => burrow(c, t, { mood: 'night', pinsFocus: true, fg: false, glow: .8 }));
    ctx.restore();
    fillPts(ctx, rect(0, 0, W, H), rgba(INK.night, .35), false);
  }
  function mug(ctx, [px, py], s) {
    const x = px - 1.55 * s, y = py - .95 * s, w = 1.5 * s, h = 1.75 * s;
    for (let k = 0; k < 3; k++) inkLine(ctx, [[x + w * (.25 + k * .25), y - .15 * s], [x + w * (.2 + k * .25) + (k - 1) * 6, y - .8 * s], [x + w * (.28 + k * .25), y - 1.4 * s]], .09 * s, rgba(INK.paper, .55), { taper: [.3, .8] });
    ...
    // carrot print
    const c = [[x + w * .5, y + h * .82], [x + w * .32, y + h * .38], [x + w * .68, y + h * .38]];
    ink(ctx, c, { fill: INK.paper, line: 3, boil: .6, smooth: false });
  ...
  function litRabbit(ctx, x, y, s, pose, M) {
    const c = pushLayer(); if (M) c.setTransform(M);
    const A = rabbit(c, x, y, s, pose); mug(c, A.pawR, s);
    for (const e of [A.eyeL, A.eyeR]) fillPts(c, rrect(e[0] + .1 * s, e[1] + .22 * s, .2 * s, .13 * s, 3), rgba(INK.cyan, .9), false);
    c.setTransform(1, 0, 0, 1, 0, 0); c.globalCompositeOperation = 'source-atop';
    fillPts(c, rect(0, 0, W, H), rgba(INK.cyan, .08), false);
    const hx = A.head[0], hy = A.head[1];
    dotsIn(c, [0, 0, W, H], { spacing: 15, color: rgba(INK.night, .8), k: (px, py) => clamp((Math.hypot(px - hx, (py - hy) * .75) - 3.4 * s) / (6 * s)) });
    c.globalCompositeOperation = 'source-over'; popLayer();
    ctx.save(); ctx.setTransform(1, 0, 0, 1, 0, 0); ctx.drawImage(c.canvas, 0, 0); ctx.restore();
    return A;
  }
  const RX = 960, RS = 80, RHY = 440, RY0 = RHY + 7.6 * RS;
  ...
  function saidCard(ctx, t) {
    ...
    const bf = { font: 'ui', weight: 700, size: 66 }, hf = { font: 'ui', weight: 800, size: 32 };
    const w = measure(ctx, 'As mentioned above.', bf).w + 120, h = 196, x = RX - w / 2, y = 834;
    uiBox(ctx, x, y, w, h, { r: 14, shadow: 10 });
    fillPts(ctx, [[RX + 30, y + 3], [RX + 90, y - 50], [RX + 86, y + 3]], INK.white, false);
    inkLine(ctx, [[RX + 30, y + 2], [RX + 90, y - 50], [RX + 86, y + 2]], 4, INK.ink, { taper: [0, 0], smooth: false });
    ...
    txt(ctx, ['As', 'mentioned', 'above.'].slice(0, n).join(' '), x + 60, y + 158, bf);
  }
  function fairy(ctx, t) {
    BOIL = 0;
    fairyBg(ctx, t);
    litRabbit(ctx, RX, RY0, RS, { ...HOLD, lids: Math.max(.5, blink(twos(t), [126.2], .2)), earL: { a: -8, b: 0 }, earR: { a: 40, b: 80 } });
    saidCard(ctx, t);
  }
  ...
  function btn(ctx, t) {
    ...
    const osc = Math.cos(n * 2.3) * Math.exp(-n * .5);   // damped ear wobble
    const pose = { ...HOLD, lids: 0, eyes: 'dot', brows: .5, sq: -.28 * osc, earL: { a: -3 - 8 * osc, b: -14 * osc, len: 1 + .22 * Math.exp(-n * .6) }, earR: { a: 3 + 8 * osc, b: 14 * osc, len: 1 + .22 * Math.exp(-n * .6) } };
    const A = litRabbit(ctx, RX, RY0, RS, pose, M);
    ctx.save(); ctx.setTransform(M);
    const ts = slam(a, [1.22, 1.07, .98, 1.01]), tx = RX, ty = 800 + (a < .09 ? 60 * (1 - a / .09) : 0), rot = -.07;
    ...
    toast(ctx, tx, ty, ts, { title: 'Small fix (again)', num: 4814, add: '28,406', del: '3', badge: 1, rot });
    ctx.restore();
  ...
  function card(ctx, t) {
    evaCard(ctx, t, [
      { s: 'CODERABBIT,', x: 120, y: 360, size: 270, sx: .66 },
      { s: 'PAUSE', x: 110, y: 900, size: 560, sx: .7 },
      { s: '\u30B3\u30FC\u30C9\u30E9\u30D3\u30C3\u30C8\u3001', x: 1800, y: 250, size: 110, font: 'jp', align: 'right', sx: .9 },
      { s: '\u4E00\u6642\u505C\u6B62', x: 1800, y: 400, size: 110, font: 'jp', align: 'right', sx: .9 },
      { s: 'EPISODE: 4814', x: 1800, y: 900, size: 150, align: 'right' },
    ], { bg: INK.ink });
  }
